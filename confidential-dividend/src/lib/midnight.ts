...
// Shared provider bundle for the dividend dApp. Mirrors the fullstack-dapp
// pattern: it lazy-imports heavy Midnight modules so the initial bundle
// stays small.
...
export async function buildProviders(opts: {
  ...
  const zkConfig = new FetchZkConfigProvider(`${CONTRACT_PATH}/managed/dividend/keys`, fetch.bind(window));
...
export async function loadCompiledContract() {
  ...
  return {
    contractModule,
    finalContract: CompiledContract.withCompiledFileAssets(ccW, `${CONTRACT_PATH}/managed/dividend`),
  };
}
